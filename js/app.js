...
// Copie novamente do console se necessário, garantindo que tenha todas as chaves
const firebaseConfig = {
    apiKey: "SUA_API_KEY",
    authDomain: "SEU_PROJETO.firebaseapp.com",
    // Para Realtime Database, essa linha abaixo é importante (se não funcionar auto, copie a URL do painel)
...
// --- SISTEMA DE SYNC (REALTIME DATABASE) ---
...
// FUNÇÃO: Busca dados no Realtime Database
async function loadUserData(id) {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            await saveData();
            showToast("Perfil criado na nuvem!", "success");
        }
        ...
        console.error("Erro RealtimeDB:", error);
        showToast("Erro ao conectar. Verifique internet/config.", "error");
    }
}

// FUNÇÃO: Salva dados no Realtime Database
async function saveData() {
    ...
    try {
        ...
        console.log("Salvo no RealtimeDB.");
    } catch (error) {
...
// Garante compatibilidade de categorias
...
function renderDashboard() {
    ...
    sortedCats.forEach(cat => {
        ...
        let dotsHtml = '';
        [4,3,2,1].forEach(u => {
            for(let i=0; i < Math.min(counts[u], 5); i++) {
                dotsHtml += `<div class="dot bg-u${u}"></div>`;
            }
        });

        card.innerHTML = `
            <i class="fa-solid ${cat.icon} main-icon"></i>
            <span class="category-name">${cat.name}</span>
            <div class="urgency-dots">${dotsHtml}</div>
            <span style="font-size: 0.7rem; color: #64748b; margin-top: 5px">${items.length} itens</span>
        `;
...
function openCategory(id) {
    ...
    if (!cat.items) cat.items = []; // Proteção contra array vazio
...
function renderItems(cat) {
    ...
    if (sortedItems.length === 0) {
        itemsList.innerHTML = `<div style="text-align:center; color:#64748b; margin-top:50px;">
    ...
    sortedItems.forEach(item => {
        ...
        div.innerHTML = `
            ...
            <div class="item-info">
                <span class="item-name">${item.name} <span class="item-badge badge-u${item.urgency}">${labels[item.urgency]}</span></span>
                <p class="item-obs">${item.obs || 'Sem observações'}</p>
            </div>
...
async function saveItem() {
    ...
    if (!name) return showToast("Digite um nome para o item", "error");
...
window.prepComplete = (idx) => confirmAction(() => completeItem(idx), "Concluir tarefa? Você ganhará 1 ponto!", "success");
...
async function completeItem(idx) {
    ...
    appData.completedItems.unshift({
        ...item,
        originalCategory: cat.id,
    ...
    renderItems(cat);
    showToast("Tarefa concluída! +1 Ponto", "success");
    await saveData();
}

// --- CONCLUÍDOS ---

document.getElementById('btn-view-completed').onclick = () => {
    renderCompletedList();
...
function renderCompletedList() {
    ...
    if(completed.length === 0) {
        completedList.innerHTML = '<p style="text-align:center; color:#64748b; padding:20px;">Nenhuma tarefa concluída ainda.</p>';
        return;
    }
    ...
    completed.forEach((item, idx) => {
        ...
        div.innerHTML = `
            <div class="item-info">
                ...
                <p class="item-obs">Concluído em: ${item.completedAt}</p>
            </div>
            <div class="item-actions">
                <button class="action-btn" title="Reabrir" onclick="restoreItem(${idx})"><i class="fa-solid fa-arrow-rotate-left"></i></button>
                <button class="action-btn btn-delete" title="Excluir Definitivo" onclick="deleteCompleted(${idx})"><i class="fa-solid fa-trash"></i></button>
            </div>
        `;
...
window.restoreItem = async (idx) => {
    ...
    if(cat) {
        ...
        renderCompletedList();
        renderDashboard(); 
        showToast("Item reaberto (Ponto removido)", "success");
        await saveData();
    }
};
...
window.deleteCompleted = (idx) => {
    confirmAction(async () => {
        ...
        renderCompletedList();
        await saveData();
    }, "Excluir do histórico permanentemente?", "danger");
};
...
function confirmAction(actionFn, text, type = 'danger') {
    ...
    iconContainer.className = 'confirm-icon';
    iconI.className = 'fa-solid';
    btnConfirm.className = '';

    if(type === 'success') {
        iconContainer.classList.add('icon-success');
        iconI.classList.add('fa-check');
        btnConfirm.classList.add('btn-success');
    } else {
        iconContainer.classList.add('icon-danger');
        iconI.classList.add('fa-trash-can');
